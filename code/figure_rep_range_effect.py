...
filename = os.path.join(os.path.expanduser('/extra/swarmstartle_results'),
                        'looming_swarm_fitted_model_high_resolution_rep_range.hdf5')
...
grid = gridspec.GridSpec(3, 3, wspace=0.2, hspace=0.2)

target_int_type = 'voronoi_matrix'

for rep_range_idx, rep_range in enumerate([1.5, 2.0, 2.5]):
    ...
    titles = ['startle freq', 'NN distance', 'pol']
    ...
    for res_idx, result_mat in enumerate([st_freqs, coh, pol]):
        ...
        for speed_idx in range(len(speeds)):
            for noise_idx in range(len(noises)):
                ...
                ax.text(noise_idx, speed_idx, str(rounded_result_mat[speed_idx, noise_idx]), color='k', weight='bold', fontsize=8,
        ...
        fig.savefig(os.path.join(figure_path, 'looming_swarm_with_speed_noise_int_type=' + target_int_type + '.pdf'), bbox_inches='tight')
        plt.close(fig)
