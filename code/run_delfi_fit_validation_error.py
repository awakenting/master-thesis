...
data_cols = ['true_rho_null', 'true_rho_null_std', 'true_noise_std_exc', 'true_rho_scale', 'mean_rho_null',
             'mean_rho_null_std', 'mean_noise_std_exc', 'mean_rho_scale','std_rho_null', 'std_rho_null_std',
             'std_noise_std_exc', 'std_rho_scale', 'final_loss', 'posterior_object', 'logs_list']
...
for dat_idx in range(nparam_sets):
    for i in range(nreps):
        ...
        logs, posterior = run_fit_with(g, n_hiddens=nhidden, n_train=ntrain, n_rounds=len(ntrain), epochs=100,
                                       data=gen_dat, seed=37)

        cmeans = posterior.mean
        cstds = posterior.std
        closs = int(10*np.log(logs[-1]['loss'][-1]))

        result_values = [gparams[dat_idx][0], gparams[dat_idx][1], gparams[dat_idx][2], gparams[dat_idx][3], cmeans[0],
                         cmeans[1], cmeans[2], cmeans[3], cstds[0], cstds[1], cstds[2], cstds[3], closs, posterior, logs]
