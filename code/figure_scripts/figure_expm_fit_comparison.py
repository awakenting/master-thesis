...
ax1.plot(expm_df['lv'], expm_df['resp_angle'], '.', color=sns_colors[0], ms=18)
ax1.plot(model_df['lv'], model_df['resp_angle'], '.', color=sns_colors[1], ms=18)
ax1.set_ylim([0, 180])
ax1.set_xlim([0, 1.3])
...
hquants_expm = ax2.plot(lv_centers - 0.05, qnt_array, ls='', marker='s', color=sns_colors[0], ms=10)
hquants_model = ax2.plot(lv_centers + 0.05, mean_model_qnt, ls='', marker='s', color=sns_colors[1], ms=10)
...
#plt.show()
plt.savefig(os.path.join(figure_path, 'figure_expm_fit_comparison.pdf'), bbox_inches='tight')