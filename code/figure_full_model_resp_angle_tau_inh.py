...
for cax, m_val in zip([ax1, ax2], [1, 2]):
    params['m'] = m_val
    tau_inh_values = [0.001, 0.002, 0.005]
    for tau_inh in tau_inh_values:
        params['tau_inh'] = tau_inh
        ...
        cax.plot(df['lv'], df['resp_angle'], '.', ms=12, label=r'$\tau_{{\rho}}$ = {:} ms'.format(tau_inh*1000))
...
#plt.show()
plt.savefig(os.path.join(figure_path, 'figure_full_model_resp_angle_tau_inh.pdf'), bbox_inches='tight')
